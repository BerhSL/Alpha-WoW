    ...
    public class Player : Unit
    {
        ...
        public void SendEquipError(InventoryError error, Item item1, Item item2)
        {
            ...
            if (error != InventoryError.EQUIP_ERR_OK)
            {
                ...
                pw.WriteUInt64(item1.Guid != 0 ? item1.Guid : this.Guid);
                pw.WriteUInt64(item2.Guid != 0 ? item2.Guid : this.Guid);
                pw.WriteUInt8(0);
            }
        ...
        public void SendSellError(SellResults error, Creature vendor, ulong item)
        {
            ...
            pw.WriteUInt64(vendor.Guid != 0 ? vendor.Guid : this.Guid);
            pw.WriteUInt64(item);
            pw.WriteUInt8((byte)error);
            this.Client.Send(pw);
        }
        ...
        public override void Update(long time)
        {
            ...
            {
                WorldObject loottarget = Database.Creatures.TryGet<WorldObject>(this.CurrentLootTarget) != null ? Database.Creatures.TryGet<WorldObject>(this.CurrentLootTarget) :
                                         Database.GameObjects.TryGet<WorldObject>(this.CurrentLootTarget);
